...
parser.add_argument("--test_folder", type=str, required=True)   # ✅ 新增参数
...
test_files = list_image_files(   # ✅ 新增 test
    args.test_folder, exts=(".jpg", ".png", ".jpeg"), follow_links=args.follow_links,
    log_progress=True, log_every_n_files=10000
)

print(f"find {len(train_files)} images in {args.train_folder}")
print(f"find {len(valid_files)} images in {args.valid_folder}")
print(f"find {len(test_files)} images in {args.test_folder}")
...
test_files = sorted(test_files)   # ✅ 排序保持一致
with open(os.path.join(args.save_folder, "test.list"), "w") as fp:
